...
func _on_upgrade_tentacles_gui_input(e : InputEvent):
	var price = 10 * upgrade_tentacle_count * upgrade_tentacle_count
	...
	if e.is_action_pressed("click"):
		if player.upgrade_tentacles(price):
			...
			$UI/upgrade_hud/_/_/_/upgrade_tentacles/price.text = "$ %d" % price

func _on_upgrade_speed_gui_input(e : InputEvent):
	var price = 10 * upgrade_speed_count * upgrade_speed_count
	...
	if e.is_action_pressed("click"):
		if player.upgrade_speed(price):
			...
			$UI/upgrade_hud/_/_/_/upgrade_speed/price.text = "$ %d" % price


func _on_upgrade_durability_gui_input(e : InputEvent):
	var price = 10 * upgrade_durability_count * upgrade_durability_count
	...
	if e.is_action_pressed("click"):
		if player.upgrade_durability(price):
			...
			$UI/upgrade_hud/_/_/_/upgrade_durability/price.text = "$ %d" % price
