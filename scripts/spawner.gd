...
func _spawn(unit : MilitaryUnit):
	...
	if unit.spawn_kind == SpawnKind.GROUND:
		unit_node.global_transform = Transform3D(Basis.IDENTITY, player.global_position).rotated(Vector3.UP, randf_range(-2*PI, 2*PI)).translated_local(Vector3(rg.randf_range(50, 150), 0, 0))
